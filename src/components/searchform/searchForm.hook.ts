...
import { pokemonDetailServices, pokemonListServices } from "@hello/sevices";
import { useEffect } from "react";
import { useForm } from "react-hook-form";
import { usePokemonListStore } from "@hello/store/pokemonList";
import { generationList } from "@hello/utils/optionlist";
...
const useSearchForm = () => {
  ...
  const callData = async (filter: {
    ...
    if (responseList.status === 200) {
      ...
      for (let pokemon of responseResults) {
